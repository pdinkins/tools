...
def csvwrite():
    with open(filestr3, 'a', newline='') as csvfile:
...
def delete():
    try:
        if len(names) == 0:
            ...
            file = open(filestr3, 'r')
            lines = file.readlines()
            file.close()
            delnum = int(input('Contact number to delete: '))
            delnum -= 1
            del lines[delnum]
            open(filestr3, 'w').writelines(lines)
            listclear()
            csvread()
    except IndexError:
        print('*** Invalid Contact Number ***')
